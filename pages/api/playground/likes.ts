...
export default async function personHandler({ body: {
    ...
    try {
        ...
        const docExists = (await firestore().collection(Collections.UserActions).doc(uid).get()).exists
        console.log('docExists------', docExists, methods)
        const promiseArr = []
        if (docExists) {
            console.log('1')
            promiseArr.push(
                firestore().collection(Collections.UserActions).doc(uid).update({
            ...
            console.log('2')
            promiseArr.push(
                firestore().collection(Collections.UserActions).doc(uid).set({
                    cardLikes: [id],
                    updatedAt: getCurrentTime(),
        ...
        console.log('3')
        promiseArr.push(
            firestore().collection(Collections.Playground).doc(id).update({
